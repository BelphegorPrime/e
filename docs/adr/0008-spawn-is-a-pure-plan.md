...
→ execute**. Everything that _decides what a run is_ is pure and testable; the
...
4. **`planSpawn(facts, resolvedModel)`** (pure) composes the whole `SpawnPlan` as
   **data**: provider delivery, MCP sidecars vs. remote vs. flag vs. file, the
   ...
   env-file's _content_ (resolving secrets by name and throwing on a missing one
...
- **Builds move up; `runSpawn` shrinks.** Image building lives in `executeSpawn`,
  ...
  invoked _inside_ `runSpawn` — a control-inversion across the seam — are gone.
...
that _also_ has a missing MCP credential.
