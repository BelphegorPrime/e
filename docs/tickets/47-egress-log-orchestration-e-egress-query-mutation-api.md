# 47 - Egress Log Orchestration + e-egress Query/Mutation API

**Status:** Done (closed 2026-09-11).

**GitHub:** [#103](https://github.com/BelphegorPrime/e/issues/103)
...
The egress container (`e-egress`, ADR-0011) is the single network gate shared by agents and MCP sidecars via `--network container:e-egress`. It runs dnsmasq (domain sinkhole) + iptables (direct-IP REJECT), reads host-mounted blacklist files, and re-applies them on SIGHUP — a host edit already takes effect immediately without restarting the shared netns.
