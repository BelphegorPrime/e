...
- `e` ships default agents — one per (harness × tier), across the tiers `default`, `smart`, `fast`, `cheap`, `review`; users add their own, mirroring how harnesses and MCP servers are shipped-plus-extendable.
