...
- **Runtime-only config over one shared per-harness image** — rejected: agents genuinely differ in *installed software* (language toolchains, extra tooling), which is a build-time concern, not data.
...
- **Hand-written derived Dockerfiles per agent** — rejected: forces the user to know Docker *and* each harness's native config format, defeating the per-harness config adapter (ADR-0006).
