...
**Agent**:
A named, reusable pairing of a Harness with a specific model configuration (provider endpoint, model, credentials) - the selectable unit a Run executes (e.g. `smart-claude`, `cheap-codex`). Many Agents can share one Harness. An Agent does not own Sidecars; those are chosen per-Run. Since ADR-0015 the type is a union: a **harness agent** as described, or a **Remote agent** (below) - both selected by name.
...
The next terms are the vocabulary of ADR-0013 (proposed; implemented through ticket 09: role contract, broker + skill, `Git.merge`, checkpoint, artifact sync, the host side that launches siblings, the merge-back, the end-to-end test, and the manual-child request source) and of ADR-0015 (the A2A task vocabulary, cancel, the event stream):
...
**Run role**:
The value of `E_ROLE` a run container is launched with: `parent` (unset or blank defaults here) or `child`, set only by the host at launch (`roleEnv`), never baked into the image and never accepted from the user (`validateSpawn` refuses `-e E_ROLE`/`-e E_BROKER_URL`). A parent run has its own runtime-broker and a `SiblingConsumer`; a child run has neither and routes its `spawn-brother` requests through the parent's broker. The launch prompt states the role; AGENTS.md guidance says to read it from `$E_ROLE`, not marker files.
**Parent run**:
A run whose role is `parent` and whose `SiblingConsumer` owns the spawn, spool and merge-back for every child it launches - broker siblings and manual children alike (ADR-0013, tickets 01-09). "Parent" is a function of the role, not of having actually spawned anything: a run is a parent the moment its `E_ROLE=parent` runtime-broker is up.
**Manual child**:
A child run requested by the host rather than by an agent: `e spawn --parent <branch> "<prompt>"` (host CLI; the serve UI and in-run exec funnel into the same path). It is written as a sibling request directly into the parent's spool - no container - and the parent's `SiblingConsumer` consumes it exactly like a broker-requested **Sibling run**: `E_ROLE=child`, same checkpoint/merge-back protocol, same report at `e-runs/<id>/report.md`. Requires a live parent (worktree + spool `run.json` naming role `parent`); refuses orphans, depth-3 callers and empty prompts.
